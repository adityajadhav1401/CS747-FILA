...
class MDP:
	...
	def lp(self):
		...
		for i, v in enumerate(problem.variables()):
			print(str(float(int(v.varValue*1e10))/1e10) + "\t" + str(policy[i]))

		# check mdp-family.txt
		# print(policy)
	...
	def hpi(self):
		...
		# print output
		for i in range(self.numStates):
			print(str(float(int(values[i]*1e10))/1e10) + "\t" + str(np.argmax(policy[i])))
		
		# check mdp-family.txt
		# print([str(np.argmax(policy[i])) for i in range(self.numStates)])
...
if __name__ == '__main__':
	...
	for i in range(len(sys.argv)):
		...
		# check mdp-family.txt
		# elif (sys.argv[i] == "--discount"):
		# 	discount = sys.argv[i+1]
